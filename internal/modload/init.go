...
import (
	...
	"io"
	"log"
	"moduleList/internal/base"
	"moduleList/internal/cfg"
	"moduleList/internal/fsys"
	"moduleList/internal/gover"
	"moduleList/internal/lazyregexp"
	"moduleList/internal/lockedfile"
	"moduleList/internal/modfetch"
	"moduleList/internal/search"
	"os"
	"path"
	"path/filepath"
	"slices"
...
func FindGoWork(wd string) string {
	...
	default:
		if !filepath.IsAbs(gowork) {
			log.Fatalf("go: invalid GOWORK: not an absolute path")
		}
		return gowork
	}
}
...
func die() {
	...
	if inWorkspaceMode() {
		log.Fatalf("go: no modules were found in the current workspace; see 'go help work'")
	}
	if dir, name := findAltConfig(base.Cwd()); dir != "" {
		...
		log.Fatalf("go: cannot find main module, but found %s in %s\n\tto create a module there, run:\n\t%sgo mod init", name, dir, cdCmd)
	}
	log.Fatal(ErrNoModRoot)
}
...
func ReadWorkFile(path string) (*modfile.WorkFile, error) {
	...
	if f.Go != nil && gover.Compare(f.Go.Version, gover.Local()) > 0 && cfg.CmdName != "work edit" {
		log.Fatal(&gover.TooNewError{What: base.ShortPath(path), GoVersion: f.Go.Version})
	}
	return f, nil
}
...
func CreateModFile(ctx context.Context, modPath string) {
	...
	if _, err := fsys.Stat(modFilePath); err == nil {
		log.Fatalf("go: %s already exists", modFilePath)
	}
	...
	if modPath == "" {
		...
		if strings.HasPrefix(modPath, "gopkg.in/") {
			...
			log.Fatalf(`go: invalid module path "%v": %v`, modPath, invalidMajorVersionMsg)
		}
...
func forceGoStmt(modFile *modfile.File, mod module.Version, v string) {
	if err := modFile.AddGoStmt(v); err != nil {
		log.Fatalf("go: internal error: %v", err)
	}
...
func commitRequirements(ctx context.Context, opts WriteOpts) (err error) {
	...
	if goVersion == "" {
		log.Fatalf("go: internal error: missing go root module in WriteGoMod")
	}
